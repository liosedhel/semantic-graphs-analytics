...
import org.virtuslab.semanticgraphs.analytics.partitions.{EdgeDTO, GraphNodeDTO, LocationDTO, PartitionHelpers, PartitionResults}
...
object PartitioningComparisonApp:
  ...
  def runPartitionComparison(projectAndVersion: ProjectAndVersion, nparts: Int, useDocker: Boolean): List[PartitionResults] =
    val projectName = projectAndVersion.projectName

    val biggestComponentNodes =
      PartitionHelpers.takeBiggestComponentOnly(SemanticCodeGraph.readOnlyGlobalNodes(projectAndVersion)).map(_.toGraphNodeDto)
