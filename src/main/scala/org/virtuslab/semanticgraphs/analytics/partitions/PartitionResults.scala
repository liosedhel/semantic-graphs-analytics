...
case class DistributionResults(nodes: List[GraphNode], nparts: Int, nodeToPart: Map[String, Int], extractGroupId: PartialFunction[GraphNode, String]) {
  ...
  private def computeDistribution(nodes: List[GraphNode], extractGroupId: PartialFunction[GraphNode, String]): List[GroupPartitionStats] = {
    nodes.collect{
...
case class PartitionResults(method: String, nodes: List[GraphNode], nparts: Int, nodeToPart: Map[String, Int], comment: String) {
  ...
  lazy val distributionVariance = (globalNodesDistribution.map(x1 => Math.pow(x1 - distributionArithmeticAverage, 2) / nparts).sum) / (distributionArithmeticAverage * distributionArithmeticAverage)


  val modularityRatio = {
    val (inner, outer) = nodes.map{ node =>
      val nodePartNumber = nodeToPart(node.id)
      val (inner, outer) = node.edges//.filter(_.`type` == "CALL")
        .filter(edge => nodeToPart.contains(edge.to)).partition(edge => nodeToPart(edge.to) == nodePartNumber)
...
object PartitionResults {
  def print(multiPrinter: MultiPrinter, results: List[PartitionResults]): Unit = {
    ...
    val longestPackageSize = results.head.packageDistribution.groupPartitionStats.maxBy(_.partitionName.length).partitionName.length
    results.foreach { result =>
      ...
      result.packageDistribution.groupPartitionStats.foreach { distribution =>
        multiPrinter.println(
          f"|${result.method.padTo(7, ' ')}|${distribution.partition}%4d|${distribution.accuracy}%7d%%|${distribution.partitionName.padTo(longestPackageSize, ' ')}|[${distribution.distribution.mkString(",")}] "
        )
      }
    }

    val longestFileSize = results.head.fileDistribution.groupPartitionStats.maxBy(_.partitionName.length).partitionName.length
    results.foreach { result =>
      multiPrinter.println(s"NParts: ${result.nparts}")
      ...
      result.fileDistribution.groupPartitionStats.foreach { distribution =>
        multiPrinter.println(
          f"|${result.method.padTo(7, ' ')}|${distribution.partition}%4d|${distribution.accuracy}%7d%%|${distribution.partitionName.padTo(longestFileSize, ' ')}|[${distribution.distribution.mkString(",")}] "
        )
      }
    }

    multiPrinter.println("Overall arithmetic average accuracy")
    multiPrinter.println("|Method |NPart|Weighted|Accuracy|Weighted|Accuracy|Variance|Modularity|Coefficient|Distribution")
    ...
    results
      .foreach { result =>
        ...
        multiPrinter.println(
          f"|${result.method.padTo(7, ' ')}|${x.nparts}%5d|${y.weightedAverageAccuracy}%7d%%|${y.arithmeticAverageAccuracy}%7d%%|${x.weightedAverageAccuracy}%7d%%|${x.arithmeticAverageAccuracy}%7d%%|${result.distributionVariance}%8.3f|${result.modularityRatio}%10.3f|${result.clusteringCoefficient}%11.3f| [${result.globalNodesDistribution.map(i => i * 100 / size).mkString(",")}]%%"
        )
      }

    multiPrinter.println("|       |     |          |           |File             |Package          |            ")
    multiPrinter.println("|Method |NPart|Modularity|Coefficient|Weighted|Accuracy|Weighted|Accuracy|Variance|Distribution")
