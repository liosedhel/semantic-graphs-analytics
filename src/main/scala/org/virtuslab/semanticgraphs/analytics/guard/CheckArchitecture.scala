...
case class ArchGuardClasses(classes: Map[String, GraphNode], allNodes: Map[String, GraphNode]) {
  def withName(name: String): ArchGuardClasses = ArchGuardClasses(
    classes.filter(_._2.displayName.contains(name)), allNodes
  )

  def inPackage(name: String): ArchGuardClasses = ArchGuardClasses(
    classes.filter(_._2.properties.get("package").exists(_.contains(name))), allNodes
  )

  def shouldNotExtend(extendNode: GraphNode => Boolean): Boolean = {
    classes.values.flatMap(_.edges).filter(_.`type` == "EXTENDS").forall(edge => allNodes.get(edge.to).forall(extendNode))
  }
}
...
case class ArchGuardMethods(workspace: String, methods: Map[String, GraphNode], allNodes: Map[String, GraphNode]) {
  def withName(name: String): ArchGuardMethods = ArchGuardMethods(
    workspace, methods.filter(_._2.displayName.contains(name)), allNodes
  )

  def inPackage(name: String): ArchGuardMethods = ArchGuardMethods(
    workspace, methods.filter(_._2.properties.get("package").exists(_.contains(name))), allNodes
  )
  ...
  def shouldNotCall(nodeCheck: GraphNode => Boolean): Boolean = {
    ...
    val a = methods.values.flatMap(_.edges).filter(_.`type` == "CALL").filter(edge => allNodes.get(edge.to).exists(nodeCheck))
    a.foreach(edge =>
      println(locationToUri(edge))
    )
    a.isEmpty
  }
}
