...
object GraphxSparkAnalysis extends App {
  ...
  val rawNodes = scg.SemanticCodeGraph.fromZip(projectToAnalyse).withoutZeroDegreeNodes().nodesMap.toSeq.map { case (key, node) =>
    val id = counter
  ...
  ShortestPaths.run(graph, stringToIntMap.values.toSeq).mapVertices((v, m) => m.map{ case (v2, value) => println(s"$v -$value-> $v2")})
