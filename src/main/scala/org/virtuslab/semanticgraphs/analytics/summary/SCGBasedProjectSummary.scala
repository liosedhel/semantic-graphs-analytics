...
object SCGBasedProjectSummary extends App {
  ...
  def exportSummary(): Unit = {
    ...
    case class SCGProjectSummary(name: String, version: String, nodes: Int, edges: Int, nodesDistribution: List[NodeKindAndNumber], edgesDistribution: List[EdgeTypeAndNumber])
    val summary = projects.map { semanticCodeGraph =>
      ...
      val edgeDistribution = semanticCodeGraph.graph.edgeSet().asScala.toList.map(_.role).groupBy(identity).map { case (role, set) => EdgeTypeAndNumber(role, set.size) }.toList.sortBy(-_.number)
      val nodesDistribution = semanticCodeGraph.nodes.map(_.kind).groupBy(identity).map { case (kind, set) => NodeKindAndNumber(kind, set.size) }.toList.sortBy(-_.number)

      SCGProjectSummary(semanticCodeGraph.project, semanticCodeGraph.version, nodesTotal, edgesTotal, nodesDistribution, edgeDistribution)
    }
  ...
  def printSummaryLatexTable() = {
    ...
    projects.map(_.withoutZeroDegreeNodes()).foreach { semanticCodeGraph =>
      ...
      println(f"${semanticCodeGraph.project} & ${semanticCodeGraph.version} & $totalLoc & $nodesTotal & $edgesTotal & $density%1.5f & $averageOutDegree%1.3f & $averageInDegree%1.3f & $globalClusteringCoefficient%1.2f & $assortativityCoefficient%1.4f\\\\")
    }
  ...
  def printStats(results: List[ProjectScoringSummary]) = {
    ...
    results.foreach { stats =>
      val topStats = stats.stats.drop(3).map { s =>
        ...
        if (topNode.score == topNode.score.toLong) {
          f"${topNode.label} & ${topNode.score}"
        } else {
          f"${topNode.label} & ${topNode.score}%.4f"
        }
      }
